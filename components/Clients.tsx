...
import { Plus, Trash2, Search, DollarSign, CheckCircle, AlertCircle, Edit2, Loader2 } from 'lucide-react';
...
const Clients: React.FC<ClientsProps> = ({ 
  ...
  // Forms State
  const [clientFormData, setClientFormData] = useState<Partial<Client>>({ type: 'avulso', status: 'active' });
  ...
  const handleOpenAddClient = () => {
    ...
    setClientFormData({ type: 'avulso', status: 'active', name: '' });
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-100">
              ...
              {displayedClients.length === 0 && <tr><td colSpan={5} className="px-6 py-12 text-center text-slate-500">Nenhum cliente encontrado</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
      ...
      <BaseModal isOpen={isClientModalOpen} onClose={() => setIsClientModalOpen(false)} title={editingClient ? 'Editar Cliente' : 'Novo Cliente'}>
        <form onSubmit={handleClientSubmit} className="space-y-4">
            ...
            <div className="flex justify-end gap-3 pt-4">
                ...
                <button type="submit" disabled={isSubmitting} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-bold flex items-center gap-2">{isSubmitting ? <Loader2 size={16} className="animate-spin" /> : 'Salvar'}</button>
            </div>
        </form>
      </BaseModal>
